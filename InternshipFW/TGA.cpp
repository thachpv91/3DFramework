...
void LoadCompressedImage( unsigned char * pDest, unsigned char * pSrc, TGA_HEADER * pHeader )
{
    int w = pHeader->width;
    int h = pHeader->height;
    int rowSize = w * pHeader->bits / 8;
    bool bInverted = ( (pHeader->descriptor & (1 << 5)) == 0 );
    unsigned char * pDestPtr = bInverted ? pDest + (h + 1) * rowSize : pDest;
    int countPixels = 0;
    int nPixels = w * h;
...
void LoadUncompressedImage( unsigned char * pDest, unsigned char * pSrc, TGA_HEADER * pHeader )
{
    int w = pHeader->width;
    int h = pHeader->height;
    int rowSize = w * pHeader->bits / 8;
...
unsigned char * LoadTGA( const char * szFileName, int * width, int * height, int * bpp )
{
    ...
    *bpp = header.bits;
    unsigned char * pOutBuffer = new unsigned char[ header.width * header.height * header.bits / 8 ];
